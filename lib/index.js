...
exports.load_dump = utils.toPromise(function (url, opts, callback) {
  var db = this;
  ...
  var processingCallback = function (err, data) {
    if (err) {
      return callback(err);
    }
  ...
  if (isUrl) {
    ajax({url: url, json: false}, processingCallback);
  } else {
    if (process) {
      var fs = require('fs');
      fs.readFile(url, { encoding: 'utf8' }, processingCallback);
    } else {
      throw 'File paths are only supported on Node environment';
    }
  }
